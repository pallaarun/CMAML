...
def CreateLoadersForAllTasks(args,train_flag):
    ...
    degradation_names = args.degradation_names.split(",") ## ["Motion","Spatial"]
...
def validate(args,epoch,model,val_task_loader,val_support_dataset,val_query_dataset,val_support_task_index,val_query_task_index,writer):
...
def main(args):
    ...
    for current_epoch in range(start_epoch,args.num_epochs):
        ...
        if str(current_epoch+1) in args.pacing_function:
            score_idx = args.pacing_function.index(str(current_epoch+1)) # if the current epoch + 1 value is present in the args. pacing function list, then we add the tasks. At the 0th epoch anyways tasks are added. args.pacing_function.index gives the index of the pacing function to access the corresponding degradation names and the task strings

            args.degradation_names = args.scored_task_strings[score_idx] # for vanilla mamal this args.degradation_names and args.task_strings are directly taken and read from the sh file. In CMAML, we assign it here based on the index of the pacing function value.
            args.task_strings = args.scored_task_strings[score_idx]
            ...
            args.no_of_train_adaptation_steps = int(args.pacing_no_of_train_adaptation_steps[score_idx])
            args.no_of_val_adaptation_steps = int(args.pacing_no_of_val_adaptation_steps[score_idx])
            ...
            print("Number of train adaptation steps =",args.no_of_train_adaptation_steps)
            print("Number of validation adaptation steps =",args.no_of_val_adaptation_steps)
            ...
            print("args.degradation_names =",args.degradation_names)
            print("args.task_strings =",args.task_strings)
        ...
        per_epoch_val_meta_loss,per_epoch_val_time = validate(args,current_epoch,model,val_task_loader,val_support_dataset,val_query_dataset,val_support_task_index,val_query_task_index,writer)
...
def create_arg_parser():
    ...
    parser.add_argument('--easy_task_strings', type=str, default='',help='All the easy tasks')
    parser.add_argument('--medium_task_strings', type=str, default='',help='All the medium tasks')
    parser.add_argument('--difficult_task_strings', type=str, default='',help='All the difficult tasks')
    
    parser.add_argument('--pacing_function',type=str, help = 'Stair case function to schedule the tasks into the loader during training')
    ...
    parser.add_argument('--pacing_no_of_train_adaptation_steps', type=str, default='', help='Pacing function for number of adaptation steps during meta-training stage')
    parser.add_argument('--pacing_no_of_val_adaptation_steps', type=str, default='', help='Pacing function for number of adaptation steps during meta-validation stage')
...
if __name__ == '__main__':
    ...
    args.pacing_function = args.pacing_function.split(",") # epoch numbers from where tasks get added 

    args.scored_task_strings = []
    args.scored_task_strings.append(args.easy_task_strings) # only easy tasks
    args.scored_task_strings.append(args.easy_task_strings+","+args.medium_task_strings) # easy + medium as a single list
    args.scored_task_strings.append(args.easy_task_strings+","+args.medium_task_strings+","+args.difficult_task_strings) # easy + medium + difficult as a single list

    args.pacing_no_of_train_adaptation_steps = args.pacing_no_of_train_adaptation_steps.split(",") # list of 3 values wherein the adaptation steps keeps increasing based on easy or medium or difficult
    args.pacing_no_of_val_adaptation_steps = args.pacing_no_of_val_adaptation_steps.split(",") # same as args.pacing_no_of_train_adaptation_steps
