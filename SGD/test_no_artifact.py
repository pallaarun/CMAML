...
def CreateLoadersForTestTasks(args):
    ...
    for one_task in full_tasks:
        ...
        support_task_loader = DataLoader(dataset = support_task_dataset, batch_size = args.test_support_batch_size,shuffle = False)
        ...
        for test_task in test_task_mini_batch:
            ...
            while True:
                ...
                for gd_steps,test_spt_data_batch in enumerate(test_support_loader[test_task]):
                    ...
                    test_spt_inp_imgs = test_spt_data_batch[1].unsqueeze(1).to(args.device)
            ...
            for _,test_qry_data_batch in enumerate(test_query_loader[test_task]):
                ...
                test_qry_inp_imgs = test_qry_data_batch[1]
                ...
                test_qry_file_names = test_qry_data_batch[6]
                break
...
def create_arg_parser():
    ...
    parser.add_argument('--checkpoint', type=pathlib.Path, required=True,help='Path to the trained model')

    parser.add_argument('--results_dir', type=pathlib.Path, required=True,help='Base path to save the reconstructions to')

    parser.add_argument('--tensorboard_summary_dir', type=pathlib.Path, required=True,help='Path to write the summary files')
    ...
    parser.add_argument('--no_of_test_adaptation_steps', type=int, default=3,help='Number of adaptation steps during meta-training stage')
