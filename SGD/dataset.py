...
class SliceData_validation(Dataset):
    ...
    def __init__(self, root):
        ...
        one_task = root.split("/")[-2]+"/"+root.split("/")[-1]
        data_specifications = ["valid_query"]
        
        for data_specification in data_specifications:
            new_root = os.path.join(root,data_specification)
            files = list(pathlib.Path(new_root).iterdir())

            for fname in sorted(files):
                with h5py.File(fname,'r') as hf:
                    crp_inp = hf['crp_inp']
                    _,_,num_slices,num_frames = crp_inp.shape
                    for frame_no in range(num_frames):
                        for slice_no in range(num_slices):
                            self.examples.append([fname,slice_no,frame_no,one_task,data_specification])
    ...
    def __getitem__(self, i):
        ...
        fname,slice_no,frame_no,one_task,data_specification = self.examples[i]

        with h5py.File(fname, 'r') as data:

            input_img  = np.abs(data["crp_inp"][:,:,slice_no,frame_no])
            target = data['crp_gt'][:,:,slice_no,frame_no]

            return torch.from_numpy(input_img),torch.from_numpy(target),slice_no,frame_no,one_task,data_specification,str(fname.name)
...
class SliceData_test(Dataset):
    ...
    def __init__(self,root,artifact_type,train_valid_support_or_query): # acc_factor can be passed here and saved as self variable
        ...
        for fname in sorted(files):
            with h5py.File(fname,'r') as hf:
                crp_inp = hf['crp_inp']
                num_slices = crp_inp.shape[2]
                num_frames = crp_inp.shape[3]

                for frame_no in range(num_frames):
                    for slice_no in range(num_slices):
                        self.examples.append((fname,slice_no,frame_no,artifact_type,train_valid_support_or_query))
    ...
    def __getitem__(self, i):
        ...
        fname,slice_no,frame_no,artifact_type,train_valid_support_or_query = self.examples[i]
        ...
        with h5py.File(fname, 'r') as data:
            ...
            return torch.from_numpy(input_img),torch.from_numpy(target),artifact_type,slice_no,frame_no,train_valid_support_or_query,str(fname)
