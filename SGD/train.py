...
def maml_validate_epoch(args,epoch,model,val_task_loader,val_support_dataset,val_query_dataset,val_support_task_index,val_query_task_index,writer):
...
def sgd_train_epoch(args,epoch,model,train_task_loader,train_support_dataset,train_query_dataset,train_support_task_index,train_query_task_index,meta_optimizer,writer):
    ...
    for iter,train_task_mini_batch in enumerate(train_task_loader):
        ...
        for train_task in train_task_mini_batch:
            ...
            for gd_steps,spt_data_batch in enumerate(train_task_support_subloader): # this loop automatically executes only for args.no_of_train_adaptation_steps times, hence an inner if block was removed
                ...
                spt_pred_imgs = model(spt_inp_imgs)
            ...
            qry_pred_imgs = model(qry_inp_imgs)
            qry_loss = F.l1_loss(qry_pred_imgs,qry_fs_imgs) # F.l1_loss will give the average loss per mini-batch of query data points in that task
            ...
            meta_grads = torch.autograd.grad(qry_loss,base_weights)
            meta_optimizer.zero_grad()
            for w,g in zip(base_weights,meta_grads):
                w.grad = g
            meta_optimizer.step()
            train_qry_loss = train_qry_loss+qry_loss # accumulate query loss per task-minibatch
            # end of one train task in train task-minibatch loop
        ...
        if iter % args.report_interval == 0:
            logging.info(
                f'Epoch = [{epoch:3d}/{args.num_epochs:3d}] '
                f'Iter = [{iter:4d}/{len(train_task_loader):4d}] '
                f'Loss = {per_train_task_minibatch_avg_qry_loss :.4g} Avg Loss = {avg_train_qry_loss:.4g} '
                f'Time = {time.perf_counter() - start_iter:.4f}s',
            )
        start_iter = time.perf_counter()
        # end of task-mini-batch for loop
    ...
    return avg_train_qry_loss,time.perf_counter()-start_epoch


def sgd_validate_wo_adaptation_epoch(args,epoch,model,val_task_loader,val_support_dataset,val_query_dataset,val_support_task_index,val_query_task_index,writer):
    ...
    for iter,val_task_mini_batch in enumerate(val_task_loader):
        ...
        for val_task in val_task_mini_batch:
            ...
            with torch.no_grad(): # this is added to ensure that gradients do not occupy the gpu mem
                ...
                val_qry_pred_imgs = model(val_qry_inp_imgs)
                unadapted_qry_loss = F.l1_loss(val_qry_pred_imgs,val_qry_tgt_imgs)
...
def sgd_validate_w_adaptation_epoch(args,epoch,model,val_task_loader,val_support_dataset,val_query_dataset,val_support_task_index,val_query_task_index,writer):
    ...
    for iter,val_task_mini_batch in enumerate(val_task_loader):
        ...
        for val_task in val_task_mini_batch:
            ...
            for gd_steps,val_spt_data_batch in enumerate(val_task_support_subloader):
                ...
                clone_grads = torch.autograd.grad(val_spt_loss,clone_weights,create_graph=False)
                #clone_grads = torch.autograd.grad(val_spt_loss, clone_weights)
                clone_weights = [w-args.adapt_lr*g for w, g in zip(clone_weights,clone_grads)]
            ...
            with torch.no_grad(): # this is added to ensure that gradients do not occupy the gpu mem
                ...
                val_qry_pred_imgs = model.adaptation(val_qry_inp_imgs,clone_weights)
                adapted_qry_loss = F.l1_loss(val_qry_pred_imgs,val_qry_tgt_imgs)
        ...
        adapted_avg_val_qry_loss = adapted_avg_val_qry_loss+adapted_val_qry_loss/len(val_task_mini_batch)
    ...
    for current_epoch in range(start_epoch,args.num_epochs):
        ...
        if args.training_style == "sgd":
            ...
            if args.adaptation_flag_for_sgd_validation_data == 'True':
                per_epoch_unadapted_val_meta_loss,per_epoch_adapted_val_meta_loss,per_epoch_val_time = sgd_validate_w_adaptation_epoch(args,current_epoch,model,val_task_loader,val_support_dataset,val_query_dataset,val_support_task_index,val_query_task_index,writer)
            elif args.adaptation_flag_for_sgd_validation_data == 'False':
                per_epoch_unadapted_val_meta_loss,per_epoch_adapted_val_meta_loss,per_epoch_val_time = sgd_validate_wo_adaptation_epoch(args,current_epoch,model,val_task_loader,val_support_dataset,val_query_dataset,val_support_task_index,val_query_task_index,writer)
            ...
            per_epoch_val_meta_loss,per_epoch_val_time = maml_validate_epoch(args,current_epoch,model,val_task_loader,val_support_dataset,val_query_dataset,val_support_task_index,val_query_task_index,writer)
