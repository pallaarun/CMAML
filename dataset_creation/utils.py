...
def normalize_4d_volume(img_vol):

    h,w,no_of_slices,no_of_frames = img_vol.shape
    norm_img_vol = np.zeros(img_vol.shape)

    for sl_no in range(no_of_slices):
        for frame_no in range(no_of_frames):
            
            norm_img_vol[:,:,sl_no,frame_no] = (img_vol[:,:,sl_no,frame_no]-np.min(img_vol[:,:,sl_no,frame_no]))/float(np.max(img_vol[:,:,sl_no,frame_no])-np.min(img_vol[:,:,sl_no,frame_no]))
